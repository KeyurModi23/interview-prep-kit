...
export const generateStructuredJSON = async (systemInstruction, prompt, schema, retries = 5) => {
  ...
  while (attempt < retries) {
    ...
    try {
      ...
      if (error.message.includes('429') || error.message.includes('quota') || error.message.includes('RESOURCE_EXHAUSTED') || error.message.includes('404') || error.message.includes('NOT_FOUND')) {
